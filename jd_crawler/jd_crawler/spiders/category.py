...
class Category(BaseSpider):
	...
	def category_parse(self, response):
		...
		if type(response) == s_response.html.HtmlResponse:
			...
			#get sku list
			p_list = hxs.select("//ul[@class='list-h']/li")
			sku_list = []
			if hxs.select("//ul[@class='list-h']/li"):
				sku_list = [p.select('./@sku').extract()[0] for p \
					in hxs.select("//ul[@class='list-h']/li")]
			if hxs.select("//div[@id='plist']/div"):
				sku_list = [p.select('./@sku').extract()[0] for p \
					in hxs.select("//div[@id='plist']/div")]
			for sku in sku_list:
				yield Request('/'.join(['http://item.jd.com', sku + '.html']), 
					callback=JD_Product_Spider().parse)
				
			next_page_link = hxs.select("//div[@class='pagin fr']/a[@class='next']/@href")
			if next_page_link:
